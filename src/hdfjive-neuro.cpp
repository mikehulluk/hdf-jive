...
SharedTimeBufferPtr SimulationResults::write_shared_time_buffer(const FloatBuffer& b)
{
    ...
    HDF5DataSet2DStdPtr pDataSet = pSimulationGroup->get_group("shared_time_arrays")->create_dataset2D(array_name, HDF5DataSet2DStdSettings(1, H5T_NATIVE_FLOAT) );
...
void SimulationResults::write_trace( const std::string populationname, int index, const std::string& record_name, SharedTimeBufferPtr times, const FloatBuffer& data, const TagList& tags )
{
    ...
    // Create the data:
    HDF5DataSet2DStdPtr pDataSet = pDataGroup->create_dataset2D("data", HDF5DataSet2DStdSettings(1, H5T_NATIVE_FLOAT) );
...
void SimulationResults::write_outputspikes( const std::string populationname, int index, const std::string& record_name, const FloatBuffer& times, const TagList& tags )
{
